...
from config import GENE2VEC_DIR
...
def vis_pca2_genevec():
    ...
    for dim in dims:
        for iter_ in iters:
            # load data
            filename = GENE2VEC_DIR + 'dim_{}/iter_{}'.format(dim, iter_)
...
def vis_scree_genevec():
    """
    Run 2d PCA on gene vectors then compare scree decompositions.
    """
    plt.figure(figsize=(18, 18))
    plt.suptitle("Scree plots on difference genevec model dimensions and iterations.")
    plt.tight_layout()
    plt.subplots_adjust(top=0.95)
...
def visualise_clusters(data_df, samples=[3000, 5000, 7000]):
    """
    ...
    axes[0,0].imshow(pd.DataFrame(np.absolute(clustered1[:,:,0])), aspect='auto')
    axes[0,0].set(title='num_clusters=5')
    axes[0,1].imshow(pd.DataFrame(np.absolute(clustered2[:,:,0])), aspect='auto')
    axes[0,1].set(title='num_clusters=10')
    axes[1,0].imshow(pd.DataFrame(np.absolute(clustered3[:,:,0])), aspect='auto')
    axes[1,0].set(title='num_clusters=20')
    axes[1,1].imshow(pd.DataFrame(np.absolute(clustered4[:,:,0])), aspect='auto')

...
def process_fasta(fname, c1, c2, filter_txt=None):
    ...
    genome = []
    for i in tqdm_notebook(chroms):
        genome.append(i.seq.__str__())
    genome = ''.join(genome).upper()
    
    print('Chunking sequence:')
    genome_chunks = []
    for i in tqdm_notebook(range(0, len(genome), c1)):
        if not 'N' in genome[i:i+c1] and set(genome[i:i+c1])==set('ATCG'):
            genome_chunks.append(genome[i:i+c1])
